...
class MLP(nn.Module):
    def __init__(self, dim, dim_out, num_layers, hidden_size = 256):
        ...
        for ind, (layer_dim_in, layer_dim_out) in enumerate(
            zip(dims[:-1], dims[1:])
        ):
...
class Dino(nn.Module):
    def __init__(
        ...
        student_temp = 0.9,
        teacher_temp = 0.04,
        local_upper_crop_scale = 0.6,
        global_lower_crop_scale = 0.6,
        ...
        self.local_crop = T.RandomResizedCrop(
            ...
            scale=(0.3, local_upper_crop_scale)
        )
    ...
    def update_moving_average(self):
        assert self.teacher_encoder is not None,\
    ...
    def forward(
        ...
        local_image_one = self.local_crop(image_one)
        local_image_two = self.local_crop(image_two)
        global_image_one = self.global_crop(image_one)
        global_image_two = self.global_crop(image_two)
        ...
        return loss, teacher_proj_one


class DinoClassifier(nn.Module):
    def __init__(self, opt, dino_model):
        super().__init__()
    ...
    def forward(self, x):
        x = self.dino_model(x, return_embedding=True, return_projection=False)
        fc = self._get_fc(x)
        return fc(x)
