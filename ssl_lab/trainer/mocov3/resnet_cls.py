...
from ssl_lab.network.mocov3.mocov3 import MoCo_ResNet_Classifier


__all__ = ['MoCoV3ResNet50ClsTrainer']


@mod.register('arch')
class MoCoV3ResNet50ClsTrainer(ResNetClsBaseTrainer):
    @gen.synchrony
    def __init__(self, opt: Opts) -> None:
        super().__init__(opt)
        ssl_model = self.load_ssl_model()
        net = MoCo_ResNet_Classifier(opt, ssl_model)
        # print(net)
        self.optimizer = SGD(
