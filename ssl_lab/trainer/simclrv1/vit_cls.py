...
from .base import ViTClsBaseTrainer
from ssl_lab.network.simclrv1.simclrv1 import SimCLR_ViT_Classifier


__all__ = ['SimCLRV1ViTClsTrainer']


@mod.register('arch')
class SimCLRV1ViTClsTrainer(ViTClsBaseTrainer):
    @gen.synchrony
    def __init__(self, opt: Opts) -> None:
        super().__init__(opt)
        ssl_model = self.load_ssl_model()
        net = SimCLR_ViT_Classifier(opt, ssl_model)
        # print(net)
        self.optimizer = SGD(
