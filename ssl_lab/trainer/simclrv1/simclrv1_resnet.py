...
from ssl_lab.network.simclrv1.simclrv1 import SimCLR_ResNet


__all__ = ['SimCLRV1ResNet50SSLTrainer']


@mod.register('arch')
class SimCLRV1ResNet50SSLTrainer(SimCLRV1BaseTrainer):
    @gen.synchrony
    def __init__(self, opt: Opts) -> None:
        super().__init__(opt)
        net = ResNet(opt, 'resnet50')
        net = SimCLR_ResNet(net)
        # print(net)
        self.optimizer = SGD(
