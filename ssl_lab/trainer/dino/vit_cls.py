...
from ssl_lab.network.dino.dino import DinoClassifier


__all__ = ['DINOViTClsTrainer']


@mod.register('arch')
class DINOViTClsTrainer(ViTClsBaseTrainer):
    @gen.synchrony
    def __init__(self, opt: Opts) -> None:
        super().__init__(opt)
        ssl_model = self.load_ssl_model()
        net = DinoClassifier(opt, ssl_model)
        # print(net)
        self.optimizer = SGD(
